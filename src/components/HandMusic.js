    ...
    hands.onResults((results) => {
      ...
      results.multiHandLandmarks.forEach((landmarks, index) => {
        ...
        if (handedness === 'Left') {
          ...
          console.log("tremoloAmount: ", tremoloAmount);
          ...
          if (distanceTipToWrist > 0.26) {
            ...
            newNote = Tone.Frequency(chromScale, "midi").toNote();
          } else if (distanceThumbToPinkyBase > 0.17) {
            let currentScale = bluesScale;
            newNote = Tone.Frequency(currentScale[vertIndex], "midi").toNote();
          }

          // Only trigger a new note if it's different from the current note
          if (newNote && newNote !== currentNote) {
            synth.triggerRelease();
            synth.triggerAttack(newNote);
            currentNote = newNote;
          } else if (!newNote && currentNote) {
            // Release note if hand gesture indicates no note should play
            synth.triggerRelease();
            currentNote = null;
          }
        }
